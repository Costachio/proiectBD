...
<div class="container">
    ...
    <form method="post" action="edit_imobil_in_database.php">
        <div class="mb-3">
            ...
            <input required type="text" name="denumire" class="form-control" value="<?php echo $imobil['denumire'] ?>"
                   id="denumire">
        </div>
        <div class="mb-3">
            <label for="adresa" class="form-label">Adresa</label>
            <input required type="text" name="adresa" class="form-control" value="<?php echo $imobil['adresa'] ?>"
                   id="adresa">
        </div>
        <div class="mb-3">
            <label for="latitudine" class="form-label">Latitudine</label>
            <input required type="text" name="latitudine" class="form-control"
                   value="<?php echo $imobil['latitudine'] ?>" id="latitudine">
        </div>
        <div class="mb-3">
            <label for="longitudine" class="form-label">Longitudine</label>
            <input required type="text" name="longitudine" class="form-control"
                   value="<?php echo $imobil['longitudine'] ?>" id="longitudine">
        </div>
        <div class="mb-3">
            ...
            <select required name="asociatie" class="form-select" id="asociatie">
                <?php foreach ($asociatii as &$asociatie): ?>
                    <option value=<?php echo $asociatie['id'] ?> <?php echo ($asociatie['id'] === $imobil['id_asociatie']) ? 'selected' : ''; ?>>
                        <?php echo $asociatie['denumire'] ?></option>
                <?php endforeach; ?>
            </select>
        </div>
